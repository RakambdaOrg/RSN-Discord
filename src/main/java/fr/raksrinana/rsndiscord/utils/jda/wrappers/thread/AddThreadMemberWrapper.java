...
public class AddThreadMemberWrapper{
	private final GuildThread thread;
	private final Object target;
	private final RestAction<Void> action;
	
	public AddThreadMemberWrapper(@NotNull GuildThread thread, @NotNull User user){
		this.thread = thread;
		target = user;
		action = thread.addThreadMember(user);
	}
	
	public AddThreadMemberWrapper(@NotNull GuildThread thread, @NotNull Member member){
		this.thread = thread;
		target = member;
