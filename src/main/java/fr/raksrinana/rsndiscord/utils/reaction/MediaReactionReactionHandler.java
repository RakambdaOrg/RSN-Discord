...
public class MediaReactionReactionHandler extends TodoReactionHandler{
	...
	protected ReactionHandlerResult processTodoCompleted(@NonNull GuildMessageReactionAddEvent event, @NonNull BasicEmotes emotes, @NonNull WaitingReactionMessageConfiguration todo){
		return Settings.get(event.getGuild()).getArchiveCategory().flatMap(CategoryConfiguration::getCategory).map(archiveCategory -> {
			...
			Actions.setCategoryAndSync(event.getChannel(), archiveCategory).thenAccept(future -> Actions.sendMessage(event.getChannel(), translate(event.getGuild(), "reaction.archived", event.getMember().getAsMention()), null));
			ChannelCommand.scheduleDeletion(ZonedDateTime.now().plusWeeks(2), event.getChannel(), event.getUser());
