...
public class AnimeMedia extends Media{
	...
	protected void fillAdditionalEmbed(@NonNull Locale locale, @NonNull EmbedBuilder builder){
		...
		Optional.ofNullable(this.getEpisodes()).map(Object::toString).ifPresent(val -> builder.addField(translate(locale, "anilist.episodes"), val, true));
		Optional.ofNullable(this.getSeason()).map(Enum::toString).ifPresent(val -> builder.addField(translate(locale, "anilist.season"), val + year.map(y -> " " + y).orElse(""), true));
	}
