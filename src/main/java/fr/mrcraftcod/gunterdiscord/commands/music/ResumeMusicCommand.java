...
public class ResumeMusicCommand extends BasicCommand{
	...
	public CommandResult execute(@NotNull final MessageReceivedEvent event, @NotNull final LinkedList<String> args) throws Exception{
		...
		switch(GunterAudioManager.resume(event.getGuild())){
			case NO_MUSIC -> Actions.reply(event, "%s, no music are currently playing", event.getAuthor().getAsMention());
			case OK -> Actions.reply(event, "%s resumed the music", event.getAuthor().getAsMention());
		}
