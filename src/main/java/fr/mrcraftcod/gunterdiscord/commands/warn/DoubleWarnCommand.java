...
public class DoubleWarnCommand extends BasicCommand
{
	...
	public CommandResult execute(MessageReceivedEvent event, LinkedList<String> args) throws Exception
	{
		...
		if(event.getMessage().getMentionedUsers().size() > 0)
		{
			...
			{
				double duration = new DoubleWarnTimeConfig().getDouble(event.getGuild(), 2);
				Actions.giveRole(event.getGuild(), user, role);
				new RemoveRoleConfig().addValue(event.getGuild(), user.getIdLong(), role.getIdLong(), (long) (System.currentTimeMillis() + new DoubleWarnTimeConfig().getDouble(event.getGuild(), 2) * 24 * 60 * 60 * 1000L));
