 ...
 * @since 2018-04-12
 */
public class WarnCommand extends BasicCommand
{
	@Override
	...
	public CommandResult execute(MessageReceivedEvent event, LinkedList<String> args) throws Exception
	{
		...
		if(event.getMessage().getMentionedUsers().size() > 0)
		{
			...
			Role role = new WarnRoleConfig().getRole(event.getGuild());
			...
			else
			{
				...
				new RemoveRoleConfig().addValue(event.getGuild(), user.getIdLong(), role.getIdLong(), System.currentTimeMillis() + new WarnTimeConfig().getLong(event.getGuild(), 1) * 24 * 60 * 60 * 1000L);
	...
	{
		return "Warn un utilisateur (en donnant un role) pendant 1 semaine";
	}
