...
public class EmoteWaitingUserReply<T> extends BasicWaitingUserReply{
	...
	public EmoteWaitingUserReply(Map<BasicEmotes, T> options, GenericGuildMessageEvent event, User author, Message infoMessage, BiConsumer<GuildMessageReactionAddEvent, T> onValue){
		...
		if(!Objects.equals(event.getUser(), event.getJDA().getSelfUser())){
			...
			if(Objects.nonNull(replyEmote)){
				if(this.options.containsKey(replyEmote)){
					final var value = options.get(replyEmote);
